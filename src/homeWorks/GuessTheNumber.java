		...
		do {
			...
			for (int i = 0; i < theNumberLength; i++) {
				...
				if (i == 0) {
					...
					boolean duplicateDigitsFound = false;
					do {
						duplicateDigitsFound = false;
						...
						for (int j = (theNumberLength - 1); (j > (theNumberLength - 1 - i) && !duplicateDigitsFound); j--) {
							tmpNum = (int) (tmptheNumber / Math.pow(10, j));
							tmptheNumber = (int) (tmptheNumber % Math.pow(10, j));
							if (randomNum == tmpNum) {
								duplicateDigitsFound = true;
							}
						}
					} while (duplicateDigitsFound);
				}
